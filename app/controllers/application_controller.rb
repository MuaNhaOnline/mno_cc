		...
		if request.get?
			...
			if session[:get_counter] == 1
				...
				unless cookies[:was_give_info]
					...
					s = Session.new
					if cookies[:1_0_begin_session_id].present?
						s.begin_session_id = cookies[:1_0_begin_session_id]
					end
					...
					if cookies[:1_0_begin_session_id].blank?
						cookies[:1_0_begin_session_id] = s.id
					end
